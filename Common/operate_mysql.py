...
class Database:
    ...
    def select_id(self, id):
        try:
            with self.connection.cursor() as cursor:
                sql = "SELECT * FROM tb_excelBean WHERE id = %s"
    ...
    def add(self, **kwargs):
        try:
            with self.connection.cursor() as cursor:
                sql = "INSERT INTO " \
                      "tb_excelBean(apiName, isRun, apiLevel, url, method, headers, requestType, requestData, remark)" \
    ...
    def delete(self, id):
        try:
            with self.connection.cursor() as cursor:
                sql = "DELETE FROM tb_excelBean WHERE id = %s"
                cursor.execute(sql, id)
                self.connection.commit()
        except:
            self.connection.rollback()
    ...
    def updata(self, **kwargs):
        try:
            with self.connection.cursor() as cursor:
                sql = "UPDATE tb_excelBean SET apiName = %s, isRun = %s, apiLevel = %s, " \
